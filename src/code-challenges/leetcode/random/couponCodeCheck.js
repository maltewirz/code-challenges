/*
The company is running a promotion in which customers receive prizes for purchasing a secret combination of fruits.
...
Output: 0
Explanation:
The customer is not a winner as the customer has added the fruits in an order which is not following the order of fruit names
in the first group.
...
Output: 0
Explanation:
The customer is not a winner as the first 2 fruits form group 1, all three fruits would form group 2, but can't because it
would contain all fruits of group 1.
...
function couponCodeCheck(codeList, shoppingCart) {
    ...
    while (codeIdx < codeList.length && shopIdx < shoppingCart.length) {
        ...
        while (codeItemIdx < codeListItem.length && shopIdx < shoppingCart.length) {
            ...
            if (codeListItem[codeItemIdx] === 'anything' ||
                codeListItem[codeItemIdx] === shoppingCart[shopIdx]
                ) {
                    codeItemIdx++
            } else {
                codeItemIdx = 0
        ...
        if (codeItemIdx !== codeListItem.length) {
            return 0
        }
        codeIdx++
    }
    if (codeList.length > codeIdx) {
        return 0
    }
    return 1
}
...
/**
 * Explanations:
1. This approach uses two pointers. First i used for loops, but while loops are needed here since sometime I need to jump back if an item is not found.

2. The run time complexity O(n^2), since the highest possible iterations are codelist.length * shoppingCart.length
*/