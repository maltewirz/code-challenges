...
# Solving Note: The functions flow is reverse:
# seven(times(five()))
# -> resolve five(): no function, therefore returns 5
# -> resolve times(5): return lambda x: x * 5
# -> resolve seven(lambda x: x * 5): return 7 if not f else f(7)
#                 return (lambda x: x* 5)(7)        (=35)
...
def times(y):
    return lambda x: x * y
    # Note: Here we are setting an abstract function 'x'


def divided_by(y):
    return lambda x: math.floor(x / y)
