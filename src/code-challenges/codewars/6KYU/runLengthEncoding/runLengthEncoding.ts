...
original string by replicating the character sx ix times and concatenating all those strings.
Your run-length encoding should be minimal, ie. for all i the values si and si+1 should differ.

Examples
As the article states, RLE is a very simple form of data compression. It's only suitable for
runs of data, as one can see in the following example:
...
export function runLengthEncoding(chars: string[]): any[] {
    ...
    for (let i = 0; i < chars.length; i++) {
        if (chars[i] === chars[i + 1]) {
            ...
            arr.push(chars[i], counter.toString());
            counter = 1;
        }
    }
    return arr;
