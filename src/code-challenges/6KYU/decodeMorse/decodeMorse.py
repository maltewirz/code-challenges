...
#    case-insensitive, traditionally capital letters are used. When
#     the message is written in Morse code, a single space is used
#      to separate the character codes and 3 spaces are used to
#       separate words. For example, the message HEY JUDE in Morse
#        code is ···· · −·−−   ·−−− ··− −·· ·.
...
#     special codes are treated as single special characters,
...
# C: provides parallel arrays, i.e. morse[2] == "-.-" for
#  ascii[2] == "C"
# All the test strings would contain valid Morse code, so
#  you may skip checking for errors and exceptions. In C#,
#   tests will fail if the solution code throws an exception,
#    please keep that in mind. This is mostly because
#     otherwise the engine would simply ignore the tests,
...
def decodeMorse(morse_code):
    ...
    for word in words:
        chars = [x for x in word.split(" ") if x]
        if chars:
            result.append("".join([MORSE_CODE[char] for char in chars]))
    return " ".join(result)
