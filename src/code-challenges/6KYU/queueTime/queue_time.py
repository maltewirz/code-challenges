...
#  task is write a function to calculate the total time required for
...
# The front person in the queue (i.e. the first element in the
...
# computer-science-related idea of a thread pool, with relation
...
def queue_time(customers, n):
    ...
    for customer in customers:
        if len(cashiers) < n:
            cashiers.append(customer)
        else:
            cashiers[cashiers.index(min(cashiers))] += customer
    return max(cashiers)
